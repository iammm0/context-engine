...
export function BboxMiniMap({ bbox, frameWidth, frameHeight, compact = false, className = "" }: BboxMiniMapProps) {
  ...
  return (
    <div
      aria-label="bbox 示意"
      className={`relative ${sizeClass} shrink-0 overflow-hidden rounded border border-dashed border-sky-200 bg-white ${className}`}
      title="bbox 示意"
    >
      <div className="absolute inset-1 rounded border border-slate-200" />
      <div
        className="absolute rounded-sm border border-sky-700 bg-sky-500/20 shadow-sm"
        style={{
          left: `${parsed.left}%`,
          top: `${parsed.top}%`,
          width: `${parsed.width}%`,
      ...
      />
      <div className="absolute inset-x-0 bottom-0 bg-white/85 px-1 py-0.5 text-[9px] leading-none text-slate-500">
        bbox
      </div>
    </div>
  )
